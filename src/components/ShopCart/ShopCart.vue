<template>
	<div>
		<div class="shopcart">
			<div class="content">
				<div class="content-left" @click="isShowToggle">
		...
		<div class="list-mask" v-show="listShow" @click="isShowToggle"></div>
	</div>
</template>
...
<script>
	...
	export default{
		...
		computed:{
			...
			...mapGetters(['totalCount','totalPrice']),		// totalCount:总数量		totalPrice:总价
			// 样式改变
			payClass(){
			...
			// bug1：当购物车列表的数量减完后，还显示“购物车，清空”字体
			// 购物车商品列表是否显示
			listShow(){
				// 如果总数量为0，就不显示
				...
				// 当购物车列表数据显示时，调用nextTick函数，创建滑动实例
				if(this.isShow){
					this.$nextTick(()=>{
						if(!this.scroll){
							...
							this.scroll.refresh();//重新计算购物车列表的高度
						}
